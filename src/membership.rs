use crate::peer::{Connector, Peer, PeerId};
use crate::view::View;
use std::sync::Arc;
    ...
    S: Sink<Bytes, Error = Error> + Send + Sync + Unpin + 'static,
{
    ...
    async fn add_active(
        ...
        if info.pid.eq(self.myself_ref()) || self.active_view().contains(&info.pid) {
            return Ok(false);
        }

        peer.send(&Message::neighbor(info.clone(), high_priority))
            .await?;
        self.passive_view_mut().remove(&info.pid);
        let removed =
            self.active_view_mut()
                .insert_replace(info.pid.clone(), peer, &mut thread_rng());
    ...
    async fn on_join(&mut self, peer: ActivePeer<S>) -> Result<(), Error> {
        let info = peer.info.clone();
        ...
        let mut failed_peers = Vec::new();
        {
            ...
            for peer in active_view.values_mut() {
                if peer.info.pid != info.pid {
                    if let Err(cause) = peer.send(&fwd).await {
                        log::warn!("couldn't send forward join to {}: {}", info.pid, cause);
                        failed_peers.push(peer.info.pid.clone());
                    }
                }
            }
        }
        ...
        for pid in failed_peers {
            if let Some(mut peer) = self.active_view_mut().remove(&pid) {
                if let Err(cause) = self.disconnect(&mut peer, true).await {
                    log::warn!("couldn't disconnect peer {}: {}", peer.info.pid, cause);
                }
            }
        }
    ...
    async fn on_disconnected(&mut self, peer: Pid, graceful: bool) -> Result<(), Error> {
    ...
    async fn on_neighbor(&mut self, peer: Arc<PeerInfo>, high_priority: bool) -> Result<(), Error> {
    ...
    async fn on_forward_join(
        ...
        if ttl == 0 || self.active_view().is_empty() {
            ...
            if ttl == self.membership_options().passive_random_walk_len {
                if !self.already_known(&peer.pid) {
                    let peer = peer.clone();
            ...
            if let Some(conn) = self
                .active_view_mut()
                .peek_value_mut(random(), |v| v.info.pid != sender)
            {
    ...
    async fn shuffle(&mut self) -> Result<(), Error> {
        ...
        if let Some(recipient) = recipient {
            ...
            let mut nodes = Vec::with_capacity(alen + plen);
            {
                let mut peers: Vec<_> = self
                    ...
                    .collect();
                if let Some(i) = peers.iter().position(|peer| peer.pid == recipient) {
                    peers.remove(i);
                }
    ...
    async fn on_shuffle_request(
        ...
        if ttl == 0 {
            ...
            for node in nodes {
                if !self.already_known(&node.pid) {
            ...
            let peer = self
                .active_view_mut()
                .peek_value_mut(random(), |v| v.info.pid != origin && v.info.pid != sender);
    ...
    fn already_known(&self, pid: &Pid) -> bool {
        self.myself_ref().eq(pid)
            || self.active_view().contains(pid)
            || self.passive_view().contains(pid)
    }

    async fn disconnect(&mut self, peer: &mut ActivePeer<S>, graceful: bool) -> Result<(), Error> {
        let myself = self.myself();
...
impl PeerInfo {
    pub fn new(pid: Pid, manifest: Manifest) -> Self {
        PeerInfo { pid, manifest }
    }

    pub fn write<W: Write>(&self, w: &mut W) {
        w.write_string(self.pid.as_ref());
